    ...
    switch (optimizer->type) {
        case SGD:
            for (int i = 0; i < num_neurons; i++) {
                sgd(fc_weights[layer_index][i], fc_grads[layer_index][i], optimizer->optimizer.sgd->momentum, optimizer->optimizer.sgd->momentum_buffer[layer_index][i], optimizer->optimizer.sgd->learning_rate);
                sgd(biases[i], bias_grads[i], optimizer->optimizer.sgd->momentum, optimizer->optimizer.sgd->momentum_buffer[layer_index][num_neurons + i], optimizer->optimizer.sgd->learning_rate);
            }
            break;
        case ADAM:
            for (int i = 0; i < num_neurons; i++) {
                adam(fc_weights[layer_index][i], fc_grads[layer_index][i], optimizer->optimizer.adam->t, optimizer->optimizer.adam->m[layer_index][i], optimizer->optimizer.adam->v[layer_index][i], optimizer->optimizer.adam->beta1, optimizer->optimizer.adam->beta2, optimizer->optimizer.adam->epsilon, optimizer->optimizer.adam->learning_rate);
                adam(biases[i], bias_grads[i], optimizer->optimizer.adam->t, optimizer->optimizer.adam->m[layer_index][num_neurons + i], optimizer->optimizer.adam->v[layer_index][num_neurons + i], optimizer->optimizer.adam->beta1, optimizer->optimizer.adam->beta2, optimizer->optimizer.adam->epsilon, optimizer->optimizer.adam->learning_rate);
            }
            optimizer->optimizer.adam->t++;
            break;
        case RMSPROP:
            for (int i = 0; i < num_neurons; i++) {
                rmsprop(fc_weights[layer_index][i], fc_grads[layer_index][i], optimizer->optimizer.rmsprop->square_avg_grad[layer_index][i], optimizer->optimizer.rmsprop->rho, optimizer->optimizer.rmsprop->epsilon, optimizer->optimizer.rmsprop->learning_rate);
                rmsprop(biases[i], bias_grads[i], optimizer->optimizer.rmsprop->square_avg_grad[layer_index][num_neurons + i], optimizer->optimizer.rmsprop->rho, optimizer->optimizer.rmsprop->epsilon, optimizer->optimizer.rmsprop->learning_rate);
            }
            break;
        default:
